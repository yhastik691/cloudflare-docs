...
- (Optional) Cloudflare R2 Access Key ID and Secret Access Key with read and write permissions. For more information, refer to [Authentication](/r2/api/s3/tokens/).
...
4. Press **Enable**, then enter details for the AWS / GCS bucket you'd like to migrate objects from. The credentials you enter must have permissions to read from this bucket. It is also a good idea to scope your credentials to only allow reads from this bucket.
5. Press **Enable**.
...
The [API documentation](/api/operations/r2-put-bucket-sippy-config) documents required parameters and examples to enable Sippy. For information about getting started with the Cloudflare API, refer to [Make API calls](/fundamentals/api/how-to/make-api-calls/).
...
The [API documentation](/api/operations/r2-delete-bucket-sippy-config) documents required parameters and examples to disable Sippy.
